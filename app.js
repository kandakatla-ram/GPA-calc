...
/* ── Weighted sample grade pool ──
   A+/A/A−/B+/B  = high probability (~77%)
   B−/C+/C       = medium probability (~15%)
   C−/D+/D/D−/F  = low probability (~8%)
*/
const SAMPLE_GRADE_POOL = [
  4.3, 4.3,
  4.0, 4.0, 4.0, 4.0,
  3.7, 3.7, 3.7,
  3.3, 3.3, 3.3,
...
/* Sample grade: weighted toward A/B range */
function sampleGrade() {
  return SAMPLE_GRADE_POOL[Math.floor(Math.random() * SAMPLE_GRADE_POOL.length)];
}

/* Count how many [sample] courses already exist */
function sampleCount(courses) {
  return courses.filter(c => c.sample).length;
}

/* Generate a "[sample]" name, with a number suffix if multiple samples exist */
function sampleName(courses) {
  const n = sampleCount(courses) + 1;
  return n === 1 ? '[sample]' : `[sample ${n}]`;
}
...
function renderChart(id, courses, gpa, maxY = 4.5) {
  ...
  const mY  = toY(gpa);
  const ticks = [0, 1, 2, 2.7, 3, 3.3, 4, 4.3].filter(t => t <= maxY + .05);
  ...
  courses.forEach((c, i) => {
    ...
    const rmFn      = c.removeFn ? `<span class="col-rm" onclick="${c.removeFn}(${c.origIdx ?? i})">✕</span>` : '';
    ...
    cols += `
      <div class="col">
        ...
        <div class="col-label">
          ...
          ${c.credits ? `<div style="font-size:8px;color:var(--green)">${c.credits}cr</div>` : ''}
  ...
  sorted.forEach(c => {
    ...
    html += `
      <div class="course-row">
        ...
        <div class="c-name">
          ${c.name}
          ${c.sample  ? '<span class="c-sample">sample</span>' : ''}
          ${c.credits ? `<span class="credits-badge">${c.credits}cr</span>` : ''}
        </div>
...
function hsAddSample() {
  hsCourses.push({ name: sampleName(hsCourses), grade: sampleGrade(), sample: true });
  hsRender(); wiRender();
}
...
function colAddSample() {
  const cr = [1, 2, 3, 3, 4, 4][Math.floor(Math.random() * 6)];
  colCourses.push({ name: sampleName(colCourses), grade: sampleGrade(), credits: cr, sample: true });
  colRender(); wiRender();
}
...
function colRender() {
  ...
  document.getElementById('col-credits-total').textContent =
    colCourses.reduce((s, c) => s + c.credits, 0) + ' credits';
...
function csAddSample() {
  const g = customScale[Math.floor(Math.random() * Math.ceil(customScale.length * 0.5))].pts;
  csCourses.push({ name: sampleName(csCourses), grade: g, sample: true });
  csRender(); wiRender();
}
...
function csRender() {
  ...
  const maxY = Math.max(...customScale.map(s => s.pts), 4.3) + .2;
  el.textContent = gpa.toFixed(2); el.style.color = gpaCol(gpa);
...
function wiRender() {
  ...
  const realGpa = wiGetGPA(base);
  const hyp     = base.map((c, i) => ({ ...c, hypGrade: wiOverrides[i] ?? c.grade }));
  const hypGpa  = wiGetGPA(hyp.map(c => ({ ...c, grade: c.hypGrade })));
